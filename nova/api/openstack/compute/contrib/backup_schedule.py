...
CONF = cfg.CONF
CONF.register_opts(backup_schedule_opts)
...
class BackupScheduleController(object):
    def __init__(self, fake=False):
        if tempo_available:
            self.tempo_client = tempo.client.TempoClient(CONF.tempo_url)
        else:
        ...
        if schedule['enabled']:
            ...
            if day_str != 'DISABLED':
                recurrence = make_recurrence(
                    minute=CONF.weekly_backup_start_minute,
                    hour=CONF.weekly_backup_start_hour,
            ...
            if hour_spec != 'DISABLED':
                recurrence = make_recurrence(
                    minute=CONF.daily_backup_start_minute,
